...
// Livres les plus empruntés (on utilise juste le titre)
livresPlusEmpruntes.clear();
for (var emprunt in emprunts) {
  String titre = emprunt.idDocument; // ton champ contient le titre
  if (titre.isNotEmpty) {
...
Text(
'Livre le plus emprunté : ${getLivreLePlusEmprunte()}',
style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 18),
),
const SizedBox(height: 16),

              // Liste des livres empruntés
              const Text('Livres les plus empruntés :',
                  style: TextStyle(fontWeight: FontWeight.bold, fontSize: 18)),
              const SizedBox(height: 8),
              if (livresPlusEmpruntes.isEmpty)
                const Text('Aucun livre emprunté')
              else
                ...livresPlusEmpruntes.entries
              ...
              const Text('Documents par catégorie :',
                  style: TextStyle(fontWeight: FontWeight.bold, fontSize: 18)),
              const SizedBox(height: 8),
              if (documentsParCategorie.isEmpty)
                const Text('Aucun document')
              else
                ...documentsParCategorie.entries
                    .map((e) => Text('${e.key} → ${e.value} documents')),

              const SizedBox(height: 16),

              // Totaux
              Text('Total emprunts : ${emprunts.length}'),
